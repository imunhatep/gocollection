...
func TestSeqMap(t *testing.T) {
	...
	doubleWithIndex := func(i int, p int) int { return p * 2 }
	...
	// map with index
	r2 := l1.MapWithIndex(doubleWithIndex)
	for i, v := range l1.ToSlice() {
		assert.Equal(t, doubleWithIndex(i, v), r2.Index(i).MustGet(), "they should be equal")
	}
}
